...
public class UserFoldAction {
	...
	public static UserFold getUserFoldTopType(int userFoldOwnerType, String userFoldOwnerId) {
		...
		try {
			...
			LogManager.mariadbLog.error("查询根文件夹失败", e);
			return null;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
	...
	public static UserFold createUserFold(UserFold userfold) {
		...
		try {
			...
			if (result != 1) {
				LogManager.mariadbLog.warn("创建文件夹失败");
				return null;
			}
			sqlSession.commit();
		} catch (Exception e) {
			...
			LogManager.mariadbLog.error("创建文件夹失败", e);
			return null;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
	...
	public static UserFold getUserFoldById(String userFoldId) {
		...
		try {
			...
			if (userFold == null) {
				LogManager.mariadbLog.warn("通过userFoldId:" + userFoldId + "获取文件夹为空");
			}
		} catch (Exception e) {
			if (sqlSession != null) {
				sqlSession.rollback();
			}
			LogManager.mariadbLog.error("获取文件夹异常", e);
			return null;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
	...
	public static UserFold updateUserFold(String userFoldId, String userFoldName, int userFoldState, boolean isUpdateUserFoldParent, String userFoldParentId) {
		...
		try {
			...
			if (result != 1) {
				LogManager.mariadbLog.warn("修改文件夹失败");
				return null;
			}
			sqlSession.commit();
		} catch (Exception e) {
			...
			LogManager.mariadbLog.error("修改文件夹异常", e);
			return null;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
	...
	public static List<UserFold> getRecyclebinUserFold(String userFoldTopId) {
		...
		try {
			...
			LogManager.mariadbLog.error("查询回收站失败", e);
			return null;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
	...
	public static boolean updateUserFoldListState(List<String> userFoldIds, int userFoldState) {
		...
		try {
			...
			LogManager.mariadbLog.error("修改文件夹状态失败", e);
			return false;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
